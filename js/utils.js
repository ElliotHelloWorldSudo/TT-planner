...
function getTeacherDisplayName(code) {
  if (!code) return "TBA";
  // Check if facultyNames exists in global scope
  if (typeof facultyNames !== 'undefined' && facultyNames[code]) {
    return facultyNames[code];
  }
  // Handle combined names like "NG / INC"
  if (code.includes('/')) {
    return code.split('/')
      .map(part => {
        const trimmed = part.trim();
        return (typeof facultyNames !== 'undefined' && facultyNames[trimmed]) ? facultyNames[trimmed] : trimmed;
      })
      .join(' / ');
  }
  return code;
